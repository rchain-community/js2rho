...
export function makeCompiler(bld) {
                ...
                case 'VariableDeclaration':
                    ...
                    switch (decl.init.type) {
                        ...
                        case 'CallExpression':
                            ...
                            return ofHarden(decl);
                        default:
                            throw todoerr('AwaitExpression | harden({...})', s0);
                    }
                default:
                    throw todoerr('VariableDeclaration', s0);
            }
        }
    ...
    function ofHarden(decl) {
        const lhs = decl.init;
        if (lhs.arguments.length !== 1) {
            throw todoerr('harden(1 arg)', decl.init);
        }
        const objectLiteral = lhs.arguments[0];
        return bld.primitive('TODO@@' + objectLiteral.type);
    }
    ...
    function ofEventualCall(js, k) {
        ...
        const dest = ofEventualTarget(js.callee);
        const procs = js.arguments.map(toProc).concat([vn(k)])
        return bld.send(dest, procs);
    }
    function ofEventualTarget(js) {
        ...
        if (js.callee.name !== 'E') {
            throw todoerr('CallExpression: E(...)', js);
        }
        if (js.arguments.length != 1) {

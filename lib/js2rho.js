...
export function makeCompiler(bld) {
    ...
    function ofModule(body) {
        ...
        return bld.new_([...newNames, rhoConsole], ofBlock(fn0.body));
    }
    ...
    function ofEventualCall(js, k) {
        ...
        const procs = [...js.arguments.map(toProc), bld.Drop(vn(k))];
        if (method === null) {
            return bld.send(target, procs);
        } else {
            return bld.send(target, [bld.primitive(method), ...procs]);
        }
    }

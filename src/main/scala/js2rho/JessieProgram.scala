...
case class FunctionExpr(n: Option[Pattern], params: List[Pattern], block: Block)
    extends Expression
...
case class Assign(op: String, lhs: Expression, rhs: Expression)
    extends Expression // TODO: AssignOp
...
case class FunctionDecl(n: Pattern, p: List[Pattern], b: Block)
    extends Declaration
...
case class ImportDeclaration(specifiers: List[ModuleSpecifier], source: StrLit)
    extends ModuleDeclaration
case class ImportSpecifier(local: Identifier, imported: Identifier)
...
case class Program(body: List[Either[Statement, ModuleDeclaration]])
    extends Positional

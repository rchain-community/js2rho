...
export function makeCompiler(bld) {
    ...
    function ofMethodSuite(decl) {
        ...
        const methods = objectLiteral.properties.map(
            ({ key, value }) => bld.contract(vn(target),
                [bld.Quote(bld.primitive(key.name))].concat(value.params.map(p => vn(p.name)), [vn("__return")]),
                //bld.primitive('TODO@@' + fresh(value.body))
                ofBlock(value.body))
        );
        return par(methods);
    }
    ...
    function ofEventualCall(js, k) {
        ...
        const { target, method } = ofEventualTarget(js.callee);
        const procs = js.arguments.map(toProc).concat([vn(k)]);
        if (method === null) {
            return bld.send(target, procs);
        } else {
            return bld.send(target, [bld.primitive(method)].concat(procs));
        }
    }
    function ofEventualTarget(js) {
        switch (js.type) {
            case 'CallExpression':
                return { target: ofE(js), method: null };
            case 'MemberExpression':
                return { target: ofE(js.object), method: js.property.name };
            default:
